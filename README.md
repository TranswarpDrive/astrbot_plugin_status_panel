...
本项目当前仓库中的实现代码，基于需求描述由 **OpenAI Codex** 完成生成与迭代，属于典型的 **vibe coding** 工作流。

就当前版本而言：

- 代码实现主要由 Codex 生成
- 仓库当前版本不包含人工手写业务代码提交
- 人工参与主要体现在需求提出、方向确认、测试反馈与迭代要求上
...
本项目在图片渲染链路的兼容性调整过程中，参考了 [exynos967/astrbot_plugin_picstatus](https://github.com/exynos967/astrbot_plugin_picstatus) 的实现思路，尤其是：

- 先在插件侧生成最终 HTML，再交给 AstrBot 的 `html_render`
- 尽量将头像等资源内联，减少 T2I 服务端再次拉取外部资源的机会

这里致谢其在 AstrBot 状态图插件方向上的实践经验。本项目的界面、数据结构和功能组织仍为当前仓库的独立实现。
